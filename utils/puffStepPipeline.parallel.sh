...
## 5kb bins, batch data, using 8x4 as ref
SMOOTHING_HALFWIDTH=3
EMISSION_MEANS="1,2,4,8,16,32,64,128"
EMISSION_SIGMAS="0.25,0.5,1,2,4,6,8,24" ##: 24 was used as last sigma for successful batch run.
BIN_SIZE=5000
EXPECTED_SPECIAL_LENGTH=1000000 ## 1Mb used successfully -- up to 10 is fine..
EXPECTED_OTHER_LENGTH=75000 ## 75 kb used successfully.

## 5kb bins, batch data, using pre-eyespot as ref
# SMOOTHING_HALFWIDTH=5 ## 3-5 ok.
# EMISSION_MEANS="1,2,4,8,16,32,64,128"
# EMISSION_SIGMAS="0.37,0.75,1,2,4,6,8,32" ##: 0.25 and 24 was used as last sigma for successful batch run. Trying to get it to work on batch and preeyspot ref.
# BIN_SIZE=5000
# EXPECTED_SPECIAL_LENGTH=15000000
# EXPECTED_OTHER_LENGTH=100000
...
######################################################################
echo "STEP 1 — MEDIAN NORMALIZE ALL SAMPLES (PARALLEL SAFE)"
######################################################################
...
shopt -u nullglob
echo "[INFO] Step 1 complete: median normalized bedGraphs written to $STEP1DIR" >&2
echo "--------------------------------------------------"
echo
...
######################################################################
echo "[INFO] Step 2 — UNION BEDGRAPH STATS + PER-BIN MEDIAN (PARALLEL)"
######################################################################
...
export UNIONBDGSTATS STEP1DIR STEP2DIR ZEROBINRM


printf "%s\n" "${GROUP_LIST[@]}" | \
parallel "${PARALLEL_OPTS[@]}" --halt soon,fail=1 '
    set -euo pipefail
    set -x
    ...
    if (( ${#files[@]} == 0 )); then
        echo "ERROR: no files for group $N in $STEP1DIR" >&2
        exit 1
    fi
    ...
    "$UNIONBDGSTATS" --check --skip-nan "${files[@]}" > "$outstats"

    [[ -s "$outstats" ]] || { echo "ERROR: empty unionstats output for group $N" >&2; exit 1; }

    awk '\''BEGIN{OFS="\t"} {print $1,$2,$3,$4}'\'' "$outstats" \
        | LC_ALL=C sortBed -i - \
        > "$outbed"

    [[ -s "$outbed" ]] || { echo "ERROR: empty median output for group $N" >&2; exit 1; }
'


shopt -u nullglob
echo "[INFO] Step 2 complete: unionBedGraphStats outputs and median bedGraphs written to $STEP2DIR" >&2
echo "--------------------------------------------------" >&2
echo >&2
...
#     awk '\''BEGIN{OFS="\t"} {print $1,$2,$3,$4}'\'' "$outstats" | LC_ALL=C sortBed -i - > "$outbed"
# ' ::: "${GROUP_LIST[@]}"
...
####################################################################################################################
echo "[INFO] Step 3 — REMOVE ZERO BINS (PARALLEL)"
####################################################################################################################
...
echo "[INFO] Step 3 complete: zero-bin-removed bedGraphs written to $STEP3DIR" >&2
echo "--------------------------------------------------" >&2
echo >&2
...
######################################################################
echo "STEP 4 — CHROMOSOME-SPECIFIC MEDIAN RATIO NORMALIZATION"
...
# --------------------------------------------------
CONTROL_FILES=( "$STEP3DIR"/1.*.bedGraph )

if (( ${#CONTROL_FILES[@]} == 0 )); then
    echo "ERROR: No control file found matching 1.*.bedGraph in $STEP3DIR" >&2
    exit 1
...
for N in "${GROUP_LIST[@]:1}"; do
    ...
    TEST_FILES=( "$STEP3DIR"/"$N".*.bedGraph )
...
echo "[INFO] Step 4 complete: chromosome-specific median ratio normalized bedGraphs written to $STEP4DIR" >&2
echo "--------------------------------------------------" >&2
echo >&2
...
######################################################################
echo "STEP 5 — MEDIAN SMOOTHING (PARALLEL + DETERMINISTIC SORT)"
######################################################################
...
echo "[INFO] Step 5 complete: median smoothed bedGraphs written to $STEP5DIR" >&2
echo "--------------------------------------------------" >&2
echo >&2
...
####################################################################################################################
echo "STEP 6 — HMM PUFFCN STATE CALLING (PARALLEL + DETERMINISTIC SORT)" >&2
####################################################################################################################
...
export PUFFSTEP EMISSION_MEANS EMISSION_SIGMAS LEAVE_SPECIAL LEAVE_OTHER STEP6DIR


# --------------------------------------------------
# PARALLEL EXECUTION
# --------------------------------------------------
echo "STEP6DIR=$STEP6DIR" >&2

: "${STEP6DIR:?STEP6DIR not set}"
: "${PUFFSTEP:?PUFFSTEP not set}"


printf "%s\n" "${files[@]}" | \
parallel "${PARALLEL_OPTS[@]}" --env PUFFSTEP --env STEP6DIR --env EMISSION_MEANS --env EMISSION_SIGMAS --env LEAVE_SPECIAL --env LEAVE_OTHER '
    set -euo pipefail
    set -x
...
echo "[INFO] Step 6 complete: HMM statepath bedGraphs written to $STEP6DIR" >&2 
echo "--------------------------------------------------" >&2
echo >&2
...
####################################################################################################################
echo "STEP 7 — COLLAPSE HMM STATE PATH BINS INTO CONTIGUOUS REGIONS WITH SAME STATE (PARALLEL + DETERMINISTIC SORT)" >&2
####################################################################################################################
...
echo "[INFO] Step 7 complete: collapsed bedGraphs written to $STEP7DIR" >&2
echo "--------------------------------------------------" >&2
echo >&2
...
####################################################################################################################
echo "STEP 8 — EXTRACT ABOVE BACKGROUND STATES (PARALLEL + DETERMINISTIC SORT)" >&2
####################################################################################################################
...
parallel "${PARALLEL_OPTS[@]}" --env STEP8DIR '
    set -euo pipefail
    shopt -s nullglob
    set -x
    ...
    awk '\''$4>1'\'' "$F" \
    | LC_ALL=C sort -k1,1 -k2,2n --stable \
...
echo "[INFO] Step 8 complete: above background region bedGraphs and merged bed files written to $STEP8DIR" >&2
echo "--------------------------------------------------" >&2
echo >&2
...
####################################################################################################################
echo "STEP 9 — EXTRACT SUMMIT STATE REGIONS (PARALLEL + DETERMINISTIC SORT)" >&2
####################################################################################################################
...
echo "[INFO] Step 9 complete: summit state region BED files written to $STEP9DIR" >&2
echo "--------------------------------------------------" >&2
echo >&2
...
####################################################################################################################
echo "STEP 10 — EXTRACT SUMMIT BINS (PARALLEL + DETERMINISTIC SORT)" >&2
####################################################################################################################
...
parallel "${PARALLEL_OPTS[@]}" '
    ...
    ## This file does not exist; use BED1 here as well.
    # if (( ${#BED2[@]} != 1 )); then
...
echo "[INFO] Step 10 complete: summit bins extracted and written to $STEP10DIR" >&2
echo "--------------------------------------------------" >&2
echo >&2
